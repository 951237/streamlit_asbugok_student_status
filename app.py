...
def main_page():
	...
	# 상단 현황판
	total_class = int(len(df['반']))
	total_man = int(df['남'].sum())
	total_woman = int(df['여'].sum())
	total_student = int(df['남'].sum() + df['여'].sum())
...
def page2():
	st.write(f"### {str_now} 현재")
	st.title(":bar_chart: 안산부곡초 전교인원 및 성별 현황")
	...
	with left:
		...
		# 학년별로 그룹지어서 전체 합계 요약
		total_student_line = (
			df.groupby(by=['학년']).sum()[['합계']].sort_values(by="합계")
		)
